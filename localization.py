...
class localization(Node):
    ...
    def __init__(self, localizationType=rawSensor):
        ...
        # TODO Part 3: subscribe to the position sensor topic (Odometry)
            self.odom_subscriber = self.create_subscription(odom, topic="/odom", callback=self.odom_callback, qos_profile=odom_qos)
        else:
    ...
    def odom_callback(self, pose_msg):
        ...
        # Log the data
        t = Time.from_msg(self.pose[3]).nanoseconds / 1e9
        self.loc_logger.log_values([self.pose[0], self.pose[1], self.pose[2], t])
...
def main():
    rclpy.init()
    example_localization = localization()
    rclpy.spin(example_localization)
    example_localization.destroy_node()
